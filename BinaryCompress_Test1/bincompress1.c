...
int setDBData (SQLHSTMT aStmt, SQL_APPEND_PARAM *sParam , unsigned long long sTime)
{
    ...
    for (idx = 1; (unsigned long)idx <= gEPS; idx++)
    {
        ...
        //data
        sParam[0].mBinary.mLength      = strlen(gData);
        sParam[0].mBinary.mData        = gData;
...
int appendData(SQLHSTMT aStmt)
{
    ...
    gData = malloc(sfSize + 1);
    memset (gData, 0x00, sfSize+1);

    fseek (sFp, 0, SEEK_SET);
    fgets (gData, sfSize, sFp);

    fprintf (stderr, "text_binary.txt Data[%s]\n", gData);
